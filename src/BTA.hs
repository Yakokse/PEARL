...
checkStep :: Step -> ST ()
checkStep (UpdateA n e1 _ e2) = do
    b1 <- checkExpr e1; b2 <- checkExpr e2
    when (b1 || b2) $ setDyn n
checkStep (UpdateV n _ e) = do
    b <- checkExpr e
    when b $ setDyn n
checkStep (Push n a) = do
    b1 <- isDyn n; b2 <- isDyn a
    when (b1 || b2) $ setDyns [a,n]
checkStep (Pop n a) = do
    b1 <- isDyn n; b2 <- isDyn a; 
    when (b1 || b2) $ setDyns [a,n]
...
checkExpr (Arr n e)    = do
    b1 <- checkExpr e; b2 <- isDyn n
    when b1 $ setDyn n
    return (b1 || b2)
checkExpr (Op _ e1 e2) = do b1 <- checkExpr e1; b2 <- checkExpr e2; return (b1 || b2)
